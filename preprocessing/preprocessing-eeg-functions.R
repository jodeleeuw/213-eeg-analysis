...
preprocess_eeg <- function(eeg.file, 
                           beh.file, 
                           subject_id, 
                           sampling_rate=500, 
                           filter_low=0.1, 
                           ...
                           eye_threshold=70, 
                           eye_electrodes = c("Fp1", "Fp2"),
                           which_electrodes=c('Cz', 'Fz', 'Fp1', 'Fp2')){
  data <- read_eeg_tidy(eeg.file, beh.file, which_electrodes) %>%
    ...
    filter_epochs_with_bad_channels(expected_channels = length(which_electrodes)) %>%
    baseline_correct(start_baseline=-200) %>%
    eyeblink_removal_with_regression(eye_channels=eye_electrodes, target_channels=setdiff(which_electrodes, eye_electrodes), blink_criteria=10, sampling_rate=sampling_rate, windows_ms=200) %>%
    change_segment_range(start_time=-200, end_time=1000) %>%
    large_anomaly_removal(max_range=200) %>%
    filter_epochs_with_bad_channels(expected_channels = length(setdiff(which_electrodes, eye_electrodes))) %>%
    baseline_correct() %>%
...
eyeblink_removal_with_regression <- function(epochs, eye_channels=c("Fp1", "Fp2"), target_channels=c("Cz", "Pz"), blink_criteria=10, sampling_rate=500, windows_ms=200){
  ...
  single_eye_channel_with_blinks <- epochs %>% 
    ...
    group_by(event_id, normal, ERP_type, t) %>%
    summarize(v = mean(v)) %>%
    group_by(event_id, normal, ERP_type) %>% # grouping by word_type and is_word just to preserve those vars
  ...
  eye_single_trials <- single_eye_channel_with_blinks %>%
    left_join(eyes_average, by=c("normal", "ERP_type", "t")) %>%
    mutate(v = v.x - v.y) %>%
    select(-v.x, -v.y)
  ...
  target_single_trials <- epochs %>% 
    dplyr::filter(electrode %in% target_channels) %>%
    left_join(target, by=c("electrode", "normal", "ERP_type", "t")) %>%
    mutate(v = v.x - v.y) %>%
    select(-v.x, -v.y)
  ...
  regression_data <- target_single_trials %>%
    left_join(eye_single_trials, by=c("event_id", "t", "normal", "ERP_type")) %>%
    dplyr::filter(is_blinking) %>%
    group_by(electrode, normal, ERP_type) %>%
    nest() %>%
  ...
  epochs.corrected <- epochs %>% 
    dplyr::filter(electrode %in% target_channels) %>%
    left_join(regression_data, by=c("electrode", "normal", "ERP_type")) %>%
    left_join(single_eye_channel_with_blinks, by=c("event_id", "t", "normal", "ERP_type")) %>%
